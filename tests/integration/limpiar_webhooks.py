...
Uso:  limpiar_webhooks.py <url-dispatcher> <token> <fqn> [--dejar-una]
"""
import json
import sys
import urllib.request

base, token, fqn = sys.argv[1], sys.argv[2], sys.argv[3]
peticion = urllib.request.Request(f'{base}/api/v1/subscriptions',
...
for sub in (propias[1:] if dejar_una else propias):
    baja = urllib.request.Request(f"{base}/api/v1/subscriptions/{sub['id']}",
