...
/**
 * Un tipo Avro crudo, en una línea legible: `string`, `int?`, `Foo[]`, `Map<string>`.
 *
 * Es lo que orienta a quien va a publicar sin tener que leer el JSON del schema entero —
 * portado de `event-gateway-ui`, donde cumple exactamente ese rol en el selector de tipos.
 */
export function formatType(type: unknown): string {
  ...
  if (type !== null && typeof type === 'object') {
    ...
    if (t.type === 'record') return String(t.name ?? '')
