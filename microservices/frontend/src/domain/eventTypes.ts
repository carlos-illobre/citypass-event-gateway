...
export type CatalogSummary = {
  total: number
  /** Sin esquema registrado todavía en el Schema Registry (`schemaId` llega null). */
  withoutSchema: number
  /** Con más de una versión mayor — alguna vez tuvo un cambio incompatible. */
 ...
 *
 * No hay estado "archivado": el gateway no da de baja lógica, borra (`DELETE`) o retira una
 * versión mayor (`DELETE .../versions/{n}`). Un tipo que aparece en el listado está vivo.
 */
export function summarizeCatalog(
  ...
  return {
    ...
    withMultipleVersions:  types.filter(t => t.versions.length > 1).length,
...
export function filterCatalog(
  types: readonly EventTypeSummary[],
  { search, namespace }: CatalogFilters,
): EventTypeSummary[] {
