...
class OpenApiConfig(private val buildProperties: BuildProperties) {
     ...
     *
     * @return Documento OpenAPI configurado para el Webhook Dispatcher.
     */
    @Bean
    fun openApi(): OpenAPI = OpenAPI()
        .info(
            Info()
                .title("CityPass+ EDA - Webhook Dispatcher")
                // Sale del build. Escrita a mano quedaba desincronizada: el documento se
                // declaraba 1.0.0 mientras el servicio corría 0.0.1-SNAPSHOT.
                ...
                .description(
                    ...
                    **Grupo 1 - Event Driven Architecture**
                    ...
                    ## ¿Dónde se crean los event types y se publican eventos?

                    En el `event-gateway`, que es otro servicio: puerto **8080** en
                    desarrollo local, con su documentación en `http://localhost:8080/doc`.
                    Acá sólo se administran las suscripciones y se consulta la cola de
                    entregas fallidas.
                    """.trimIndent()
                )
        )
     ...
     *
     * Todos los errores de este servicio son RFC 9457, así que describirlos uno por uno en cada
