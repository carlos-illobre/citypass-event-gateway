...
class SubscriptionService(
    ...
    @Value("\${dispatcher.data-dir}") private val dataDir: String,
    @Value("\${dispatcher.max-webhooks-per-event-type}") private val maxPorEventType: Int
) {
     ...
     */
    /**
     * Una suscripción por su id, sin filtrar por dueño.
     *
     * Quien llama tiene que comprobar el `owner`: acá no se hace porque hay usos legítimos
     * que necesitan la suscripción antes de saber si corresponde —la reentrega de la DLQ
     * compara el dueño guardado contra el vigente, y para eso necesita los dos—.
     */
    fun getById(id: String): Subscription? = subscriptions[id]
    ...
    internal fun consumerProps(topic: String): Map<String, Any> =
        consumerFactory.configurationProperties.toMutableMap().apply {
            ...
            // El nombre del servicio que consume, que es este. El group id es lo que
            // aparece en `kafka-consumer-groups` y en kafka-ui cuando hay que diagnosticar
            // un tópico frenado, así que tiene que decir quién está atrás del consumer.
            //
            // Renombrarlo desde `event-gateway-webhook-` fue un corte deliberado: un group
            // id nuevo no tiene offsets, así que la primera vez que se despliega este
            // cambio cada suscripción arranca desde el final y se saltea lo publicado
            // entre el reinicio y la primera asignación de particiones. Es por única vez y
            // está anunciado en el ADR-020; el precio de arrastrar un nombre que miente es
            // permanente.
            put(ConsumerConfig.GROUP_ID_CONFIG, "webhook-dispatcher-$topic")
    ...
    internal fun dispatch(record: ConsumerRecord<String, ByteArray>) {
        ...
        try {
            ...
            // el offset se confirma recién cuando este método vuelve: si el dispatcher muere
