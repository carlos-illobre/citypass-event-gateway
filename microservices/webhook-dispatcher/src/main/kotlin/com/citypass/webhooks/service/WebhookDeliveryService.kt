 ...
 * confirme recién cuando el evento está entregado o registrado en la DLQ; si el dispatcher
...
class WebhookDeliveryService(
    ...
    @Value("\${dispatcher.webhook-failures-before-disable}") private val fallosParaSilenciar: Int,
    @Value("\${dispatcher.webhook-disable-minutes}") private val minutosSilenciada: Long
) {
    ...
    //
    // El estado vive en memoria y no en el disco a propósito: reiniciar el dispatcher
    ...
    fun deliverAll(subscriptions: Collection<Subscription>, event: Map<String, Any?>) {
        ...
        // Se resuelve el instante una sola vez por suscripción. Consultarlo de nuevo dentro
        // del bucle obligaba a un `?: Instant.now()` que nunca podía ejecutarse.
        val silenciadas = subscriptions.mapNotNull { sub -> silenciadaHasta(sub.id)?.let { sub to it } }
        val activas = subscriptions.filter { silenciadaHasta(it.id) == null }

        // Se omite la entrega pero **no** el registro: el evento va a la DLQ igual, para
        // que se pueda reentregar cuando el destino vuelva. Antes se perdía con sólo un
        // contador, y era la pérdida más grande de las dos —el cortacircuitos se activa
        // justo cuando el destino lleva rato caído, así que se llevaba la ráfaga entera—.
        silenciadas.forEach { (sub, hasta) ->
            logger.debug("Webhook ${sub.callbackUrl} silenciado, se omite la entrega")
            contar(sub.topic, "omitido")
     ...
     * @param registrarEnDlq Si al agotar los intentos hay que dejar el evento en la DLQ.
     *   Es `false` en una **reentrega manual**: el evento ya está en la DLQ, y volver a
     *   escribirlo agregaría una copia por cada botón apretado. Cinco reintentos dejarían
     *   cinco entradas del mismo evento y la pantalla dejaría de ser legible.
     */
        ...
        for (attempt in 1..maxRetries) {
            ...
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    if (!registrarEnDlq) return false
                    dlqService.sendWebhookFailure(
                        originalTopic = subscription.topic,
                        originalKey = null,
                        eventJson = event,
