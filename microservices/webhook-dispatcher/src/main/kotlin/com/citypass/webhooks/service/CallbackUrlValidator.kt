 ...
 *
 * El dispatcher hace un POST a la `callbackUrl` de cada suscripción. Sin esta
 * verificación, cualquier grupo autenticado puede hacer que el dispatcher golpee
 ...
 * Kafka, o el gateway. Es un SSRF con el dispatcher de proxy.
 *
 ...
 * una privada cuando el dispatcher entrega.
 *
...
open class CallbackUrlValidator(
    @Value("\${dispatcher.allow-private-callbacks}") private val allowPrivate: Boolean
) {
    ...
    init {
        if (allowPrivate) {
            logger.warn(
                "dispatcher.allow-private-callbacks=true: se aceptan webhooks hacia direcciones " +
    ...
    fun reject(callbackUrl: String): String? {
        ...
        val addresses = runCatching { resolve(host) }.getOrNull()
            ?: return "No se pudo resolver el host '$host'. La callbackUrl tiene que ser " +
                "alcanzable desde donde corre el dispatcher."

        val interna = addresses.firstOrNull { isInternal(it) }
            ?: return null

        return "'$host' resuelve a ${interna.hostAddress}, que es una dirección de red " +
            "interna. La callbackUrl tiene que ser una URL pública: el dispatcher no corre " +
    ...
    private fun isInternal(address: InetAddress): Boolean =
        address.isLoopbackAddress ||      // 127.0.0.0/8, ::1 — el propio dispatcher
