...
class SubscriptionServiceTest {
    ...
    fun `el consumer se crea con commit manual y ack por registro`() {
        ...
        assertEquals("webhook-dispatcher-topic.durable",
    ...
    @Test
    fun `getById devuelve la suscripcion sin filtrar por dueno`() {
        val creada = subscriptionService.register(
            "com.citypass.movilidad.BiciDevuelta", "https://destino.example/hook",
            "com.citypass.movilidad", "grupo3"
        ).getOrThrow()

        // Sin filtrar por dueño a propósito: quien llama compara el owner guardado contra
        // el vigente, y para eso necesita poder traer la suscripción primero.
        assertEquals(creada.id, subscriptionService.getById(creada.id)?.id)
        assertNull(subscriptionService.getById("no-existe"))
    }
}
