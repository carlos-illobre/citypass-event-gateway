...
class WebhookDeliveryServiceTest {
    ...
    fun `el silencio vence solo`() {
        ...
        // pegado hasta el próximo reinicio del dispatcher.
    ...
    /**
     * Lo que antes se perdía: con la suscripción silenciada, el evento no se intenta
     * entregar **pero sí se registra**, para poder reentregarlo cuando el destino vuelva.
     *
     * Es la pérdida más grande de las dos, porque el cortacircuitos se activa justo cuando
     * el destino lleva rato caído: se llevaba la ráfaga entera de esa ventana.
     */
    @Test
    fun `una suscripcion silenciada manda el evento a la DLQ en vez de perderlo`() {
        val servicio = service(fallosParaSilenciar = 1)
        fallar(servicio, veces = 1)
        assertNotNull(servicio.silenciadaHasta(subscription.id))
        clearInvocations(dlqService)

        servicio.deliverAll(listOf(subscription), event)

        verify(dlqService).sendWebhookSilenciado(
            originalTopic = eq("test.topic"),
            originalKey = isNull(),
            eventJson = eq(event),
            subscriptionId = eq(subscription.id),
            callbackUrl = eq(subscription.callbackUrl),
            hasta = any(),
            owner = eq("com.citypass.movilidad")
        )
        assertEquals(1.0, entregas("omitido"))
    ...
    /**
     * La reentrega manual no vuelve a escribir en la DLQ: la entrada ya existe.
     *
     * Sin esto, apretar «reintentar» cinco veces dejaría cinco copias del mismo evento en
     * el listado, y la pantalla que sirve para resolver el problema pasaría a ser parte de
     * él.
     */
    @Test
    fun `con registrarEnDlq en false, agotar los reintentos no duplica la entrada`() {
        whenever(restClient.post()).thenThrow(RuntimeException("connection refused"))

        val entregado = service().deliverWithRetry(
            subscription, event, maxRetries = 2, retryDelayMs = 0, registrarEnDlq = false
        )

        assertEquals(false, entregado)
        verifyNoInteractions(dlqService)
        assertEquals(1.0, entregas("agotado"))
    }
}
