...
C4Container
    ...
    System_Ext(grupo3, "Grupo 3 — Movilidad", "Publica eventos de com.citypass.movilidad y los consume")
    System_Ext(grupo4, "Grupo 4 — Reclamos", "Publica eventos de com.citypass.reclamos y los consume")
    System_Ext(grupoN, "Grupos 5-8", "Publican y consumen eventos de sus dominios")

    Rel(cliente, proxy, "HTTPS y Kafka sobre TLS", "Único punto de entrada en producción")
    Rel(proxy, event_gateway, "HTTP", "Tráfico ya descifrado")
    Rel(proxy, kafka, "TCP", "Listener SASL del broker")
    Rel(gateway_ui, event_gateway, "HTTP/REST", "JSON")
